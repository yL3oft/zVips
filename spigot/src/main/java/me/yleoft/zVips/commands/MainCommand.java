...
import me.yleoft.zVips.storage.Messages;
...
public class MainCommand extends ConfigUtils implements CommandExecutor {
    public boolean onCommand(@NotNull CommandSender s, @NotNull Command cmd, @NotNull String label, String[] args) {
        ...
        switch (subcmd.toLowerCase()) {
            ...
            case "converter":
                ...
                zVipsBukkit.db.migrateData(p, args[1]);
                return false;
            case "version":
            case "ver":
                ...
                if(args.length > 1) {
                    ...
                    switch (subcmd2) {
                        ...
                        case "savekey": {
                            ...
                            int id = zVipsBukkit.dbe.setKey(gkey);
                            KEY ckey = zVipsBukkit.dbe.getKey(id);
                        ...
                        case "transferkey": {
                            ...
                            KEY keyO = zVipsBukkit.dbe.getKey(id);
                        ...
                        case "listkeys": {
                            ...
                            if (t == null) {
                                ...
                                if(args.length == 4 && args[3].equalsIgnoreCase("--usable")) {
                                    ...
                                    zVipsBukkit.dbe.getUsableKeys(t).forEach(key -> lang.sendMsg(s, "&9- Key: &a" + key.getKey() + " &7(ID: " + key.getID() + " O: " + (key.getOwner() == null ? "false" : "true") + ")"));
                                    return true;
                                }
                                lang.sendMsg(s, "&9[LIST] " + t.getName() + "'s keys:");
                            }
                            zVipsBukkit.dbe.getKeys(t).forEach(key -> lang.sendMsg(s, "&9- Key: &a" + key.getKey() + " &7(ID: " + key.getID() + ")"));
                            break;
                        }
                        case "listvips": {
    ...
    public String getUsage(CommandSender s, LanguageUtils.MainCMD.MainHelp lang) {
        if (s instanceof Player) {
            ...
            if (p.hasPermission(CmdMainHelpPermission())) {
                System.out.println(Messages.MainCMD.Help.HELP_PERM);
                return lang.getUsageWithPerm();}
            System.out.println(Messages.MainCMD.Help.HELP_NOPERM);
    ...
    public long reload(String which) {
        ...
        switch (which) {
            ...
            case "commands":
                reload("config");
                zVipsBukkit.getInstance().loadCommands();
                PluginYAMLManager.syncCommands();
                break;
            case "config":
                zVipsBukkit.getInstance().reloadConfig();
                zVipsBukkit.cfgu = new ConfigUtils();
                break;
            case "languages":
