...
public class ConfigUtils {
    ...
    public static class ConfigUtilsExtras {
        ...
        public boolean canAfford(Player p, String commandPermission, Float cost) {
            if(p.hasPermission(zVipsBukkit.cfgu.PermissionBypassCommandCost(commandPermission))) {
                return true;
            }
            Economy economy = (Economy) zVipsBukkit.economy;
            LanguageUtils.HooksMSG hooks = new LanguageUtils.HooksMSG();
            if (zVipsBukkit.getInstance().getServer().getPluginManager().isPluginEnabled("Vault")) {
